...
bool fft_settled(void)
{
    ...
    for (size_t i = 0; i < N; ++i) {
        if (plug->out_smooth[i] > eps) return false;
        if (plug->out_smear[i] > eps) return false;
    }
    return true;
}
...
void fft_clean(void)
{
    memset(plug->in_raw, 0, sizeof(plug->in_raw));
    memset(plug->in_win, 0, sizeof(plug->in_win));
    memset(plug->out_raw, 0, sizeof(plug->out_raw));
    memset(plug->out_log, 0, sizeof(plug->out_log));
    memset(plug->out_smooth, 0, sizeof(plug->out_smooth));
    memset(plug->out_smear, 0, sizeof(plug->out_smear));
}
...
size_t fft_analyze(float dt)
{
    ...
    for (size_t i = 0; i < N; ++i) {
        ...
        plug->in_win[i] = plug->in_raw[i]*hann;
    }

    // FFT
    fft(plug->in_win, 1, plug->out_raw, N);
    ...
    for (size_t i = 0; i < m; ++i) {
        plug->out_log[i] /= max_amp;
    }

    // Smooth out and smear the values
    for (size_t i = 0; i < m; ++i) {
        float smoothness = 8;
        plug->out_smooth[i] += (plug->out_log[i] - plug->out_smooth[i])*smoothness*dt;
        float smearness = 3;
        plug->out_smear[i] += (plug->out_smooth[i] - plug->out_smear[i])*smearness*dt;
    }
...
void fft_render(size_t w, size_t h, size_t m)
{
    ...
    for (size_t i = 0; i < m; ++i) {
        float start = plug->out_smear[i];
        float end = plug->out_smooth[i];
        float hue = (float)i/m;
        Color color = ColorFromHSV(hue*360, saturation, value);
        Vector2 startPos = {
            i*cell_width + cell_width/2,
...
{
    memmove(plug->in_raw, plug->in_raw + 1, (N - 1)*sizeof(plug->in_raw[0]));
    plug->in_raw[N-1] = frame;
}
